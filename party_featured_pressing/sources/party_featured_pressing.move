...
/// profile.
///
/// A party's full discography is *derived* (an indexer lists every pressing the
...
/// `Pressing<Currency>` object, so the featured id is *proven* to be a real
/// pressing at write time — no garbage or dangling ids can be pinned. A
/// `Pressing` is a shared, `key`-only, currency-generic object, so it can't be
/// held on the party; we store only its `ID`. The pressing's live state (price,
...
/// Dynamic-field key for a party's featured-pressing id.
public struct FeaturedKey() has copy, drop, store;

// === Events ===

/// Emitted when a party's featured pressing is set or replaced.
public struct FeaturedSetEvent has copy, drop {
    party_id: ID,
    pressing_id: ID,
}

/// Emitted when a party's featured pressing is cleared.
...
/// `Pressing` object so the stored id is guaranteed to be a real pressing; only
/// its id is kept.
public fun set_featured<Currency>(
    self: &mut Party,
    cap: &PartyAdminCap,
    pressing: &Pressing<Currency>,
) {
    let party_id = self.id();
    let pressing_id = pressing.id();
    let uid = self.uid_mut(cap);
    if (df::exists(uid, FeaturedKey())) {
        *df::borrow_mut(uid, FeaturedKey()) = pressing_id;
    } else {
        df::add(uid, FeaturedKey(), pressing_id);
    };
    emit(FeaturedSetEvent { party_id, pressing_id });
}
